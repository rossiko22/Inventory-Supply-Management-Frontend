...
export default function NewVehicleScreen(): React.ReactElement {
  const router      = useRouter();
  const queryClient = useQueryClient();
  const [submitError, setSubmitError] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: (values: VehicleFormValues) => fleetApi.createVehicle(values),
    onSuccess: () => {
  ...
  return (
    <SafeAreaView style={styles.root}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>{sl.fleet.newVehicle}</Text>
        <View style={{ width: 60 }} />
      </View>
      <VehicleForm
