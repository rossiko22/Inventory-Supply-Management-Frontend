...
export default function NewDriverScreen(): React.ReactElement {
  const router      = useRouter();
  const queryClient = useQueryClient();
  const [submitError, setSubmitError] = useState<string | null>(null);

  const mutation = useMutation({
    mutationFn: (values: DriverFormValues) => fleetApi.createDriver(values),
    onSuccess: () => {
  ...
  return (
    <SafeAreaView style={styles.root}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>{sl.fleet.newDriver}</Text>
        <View style={{ width: 60 }} />
      </View>
      <DriverForm
